...
class InputController(QObject):
    ...
    def type_text(self, text):
        ...
        self.is_typing = True
        # Đợi lâu hơn để đảm bảo phím Ctrl được nhả ra
    ...
    def _type_with_clipboard(self, text):
        ...
        try:
            ...
            # Đảm bảo Ctrl không được nhấn (có thể vẫn đang được nhấn nếu người dùng chưa nhả)
            while keyboard.is_pressed('ctrl'):
                logger.debug("Đang đợi phím Ctrl được nhả...")
                time.sleep(0.1)
                
            # Dán văn bản vào vị trí con trỏ hiện tại
    ...
    def check_ctrl_state(self):
        try:
            ...
            if is_pressed != self.last_ctrl_state:
                ...
                logger.debug(f"Trạng thái phím Ctrl thay đổi: {is_pressed}")
                self.ctrl_pressed.emit(is_pressed)
        except Exception as e:
            logger.exception(f"Lỗi khi kiểm tra trạng thái phím Ctrl: {e}")
